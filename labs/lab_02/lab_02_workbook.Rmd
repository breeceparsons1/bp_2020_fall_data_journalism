...
```{r}
# Answer to question 1: Giant Pharmacy 166 with 10365 shipments 
arcos_md_baltimorecity_pharmacies <- arcos_md_baltimorecity %>% select(buyer_name, total_records) %>% arrange(desc(total_records))
...
```{r}
# Answer to question 2: Oshinsky, Alan E MD and Jakobovits, Julian MD both have the lowest number of pills with 4 each.  
arcos_md_baltimorecity_pharmacies <- arcos_md_baltimorecity %>% select(buyer_name, total_dosage_unit) %>% arrange(total_dosage_unit)
```
