...
## Description: This block will examine which honoree has the most monuments listed to their names out of the entire list and then out of the names highlighted in the data.
...
## Description: This block will determine which states have the most symbols and specifically, monuments and how the number’s changed over time 

#Finding which states have the most symbols 

```{r}
states_total <- heritage %>%
  clean_names() %>%
  group_by(state) %>%
  count() %>%
  arrange(desc(n)) 
...
#A: VA has the most with 301, followed by Texas with 242. 

#Adding a filter column to specifically look at monuments 
...
#A: Found VA had most with 119 and GA had second most with 113 
...
```
#A: Found VA had the most still with 126 and Texas next with 110.  
...
```
#A: Found VA had most with 97. Second, GA and NC both had 79  
