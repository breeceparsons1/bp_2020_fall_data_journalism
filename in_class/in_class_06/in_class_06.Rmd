...
```{r}
most_covid_cases_state <- state_covid_population %>%
  select(state, total_cases,population) %>%
  mutate(case_per_1000_august = ((total_cases/population)*1000)) %>%
  arrange(desc(case_per_1000_august))
...
most_covid_deaths_state <- state_covid_population %>%
